...
function generateMongoDBURL(options) {
  ...
  if (username && options.password) {
    ...
    return options.protocol + '://' + options.hostname + portUrl + '/' + options.database;
  }
}
...
MongoDB.prototype.connect = function (callback) {
  ...
  if (self.db) {
    ...
    console.log(self.settings.url, validOptions);
    ...
    client
      ...
      .then(function () {
        ...
        self.db = client.db(url.dbName || self.settings.database, validDbOptions);
        if (callback) {
...
MongoDB.prototype.create = function (modelName, data, options, callback) {
  ...
  this.execute(
    ...
    'insertOne',
    data,
    buildOptions({ safe: true }, options),
    function (err, result) {
      if (self.debug) {
...
MongoDB.prototype.save = function (modelName, data, options, callback) {
  ...
  this.execute(
    ...
    { _id: oid },
    { $set: data },
    buildOptions({ upsert: true }, options),
    function (err, result) {
      if (!err) {
...
MongoDB.prototype.parseUpdateData = function (modelName, data, options) {
  ...
  if (allowExtendedOperators) {
    ...
    for (let i = 0; i < ACCEPTED_UPDATE_OPERATORS.length; i++) {
      if (data[ACCEPTED_UPDATE_OPERATORS[i]]) {
  ...
  Object.keys(where).forEach(function (k) {
    ...
    if (spec) {
      ...
      if (spec === 'between') {
        ...
        if (cond.global) g.warn('{{MongoDB}} regex syntax does not respect the {{`g`}} flag');
...
MongoDB.prototype.getDatabaseColumnName = function (model, propName) {
  ...
  if (prop.id) {
    ...
    if (customFieldName) {
      throw new Error(
        `custom id field name '${customFieldName}' is not allowed` +
...
MongoDB.prototype.all = function all(modelName, filter, options, callback) {
  ...
  function processResponse(err, cursor) {
    ...
    cursor
      ...
      .then(function (data) {
        if (self.debug) {
          debug('all', modelName, filter, data);
        }
        const objs = self.toModelEntity(modelName, data, fields);

        if (filter && filter.include) {
          self._models[modelName].model.include(objs, filter.include, options, callback);
        } else {
          callback(null, objs);
        }
      })
      .catch(function (err) {
        if (self.debug) {
          debug('all Error', modelName, filter, err);
        }
        callback(err);
      });
  }
...
MongoDB.prototype.replaceById = function replace(modelName, id, data, options, cb) {
  ...
  const oid = this.coerceId(modelName, id, options);
  this.replaceWithOptions(modelName, oid, data, { upsert: false }, function (err, data) {
    cb(err, data);
  });
};
        ...
        for (const p in properties) {
          if (properties[p].index) {
            ...
            if (typeof properties[p].index === 'object') {
              ...
              const indexType =
                typeof properties[p].index === 'string' ? properties[p].index : '2dsphere';
        ...
        async.eachSeries(
          ...
          function (index, indexCallback) {
            ...
            self
              ...
              .createIndex(index.fields || index.keys, index.options, indexCallback);
          },
          modelCallback
        );
      },
      ...
      function (modelName, modelCallback) {
        ...
        self.db.dropCollection(collectionName, function (err, collection) {
          if (err) {
            ...
            if (
              !(err.name === 'MongoServerError' && err.ok === 0 && err.errmsg === 'ns not found')
            ) {
...
function isStrictObjectIDCoercionEnabled(modelCtor, options) {
  ...
  return (
    ...
    (options && options.strictObjectIDCoercion)
  );
}
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  } else if (value instanceof mongoObjectID) {
    return true;
  } else {
    return false;
  }
...
function optimizedFindOrCreate(modelName, filter, data, options, callback) {
  ...
  this.collection(modelName)
    ...
    .then((result) => {
      ...
      if (filter && filter.include) {
        self._models[modelName].model.include([value], filter.include, function (err, data) {
...
function visitAllProperties(data, modelCtor, visitor) {
  ...
  for (const p of allProps) {
    ...
    if (def && def.type && isNestedModel(def.type)) {
      ...
      visitor(modelCtor, value, def, (newValue) => {
        data[p] = newValue;
      });
    }
    continue;
...
function buildOptions(requiredOptions, connectorOptions) {
  if (connectorOptions && connectorOptions.transaction && connectorOptions.transaction.isActive()) {
    return Object.assign(
      { session: connectorOptions.transaction.connection },
      connectorOptions,
      requiredOptions
    );
  } else {
